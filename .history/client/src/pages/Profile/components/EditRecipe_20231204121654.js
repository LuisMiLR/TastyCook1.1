...
const EditRecipe = () => {
  ...
  useEffect(() => {
    ...
    if (token && id) {
      getOnePost(id, token) // avec ID de la recette extrait des paramètres de l'URL, et token
        .then((res) => {
          setRecipe(res.post);
  ...
  const handleUpdate = async () => {
    ...
    setIsLoading(true);
    try {
      const formData = new FormData();// create new form data object to send to backend
      ...
      await updatePost(recipe, id, token); // get l'ID de la recette pour la mise à jour
