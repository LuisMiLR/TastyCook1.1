...
const EditRecipe = () => {
  const { id } = useParams(); // Utilisez useParams pour accéder aux paramètres de l'URL
  ...
  useEffect(() => {
    ...
    if (token && id) {
      getOnePost(id, token) // Utilisez l'ID de la recette extrait des paramètres de l'URL
        .then((res) => {
          setRecipe(res.post);
          setIsLoading(false);
        })
